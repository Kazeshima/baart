...
Dimension score weights are stored as normalized percentages that always add up to 100%. Shared weights are the default so every student uses the same scoring formula; an Individual mode remains available for per-student exceptions. Each weight can be adjusted by slider or typed percentage input. Moving one equalizer-style control redistributes the opposite change across the other four dimensions in proportion to their current weights, and the score is calculated as `sum(score × percentage) / 100`. Older JSON files that contain `none`, `half`, or `full` weights are migrated automatically into equivalent normalized shares.
