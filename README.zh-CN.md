...
五个维度权重提供两种编辑方式。默认的“精细百分比”使用固定 `0-100%` 刻度和明确的“未分配权重”预算：提高某个维度时只会消耗未分配预算，降低某个维度时会把释放的权重放回未分配预算，其他维度不会自动变化。百分比输入框只会在失焦或按 Enter 时提交，输入过程中的半成品数值不会立即生效；超过可用预算的数值只会夹紧当前编辑的维度。当未分配权重不为 `0%` 时，自动综合分、卡片导出和视频渲染会暂时禁用；完整分配后按 `sum(维度分 × 权重百分比) / 100` 计算。“预设权重”会恢复旧版的不计/半权重/全权重系统，适合不想细调的用户。默认仍使用全体共用权重，也可切换到单独设置模式。评级编辑器和视频工作室使用同一套共用权重设置，因此在任意页面修改都会更新同一份底层状态。已有归一化百分比 JSON 会按精细百分比导入，旧版 `none`、`half`、`full` 文件会按预设权重导入。
