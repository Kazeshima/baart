...
视频工作室可从本地存储或评级 JSON 读取数据。`.baart-video.json` 项目清单会保存标准化评级、已解析的学生资料、手动顺序、语言、主题、时间、特效、分辨率和输出设置，以便重复生成相同结果。学生可按评级时间、综合分、ID、学校分组排序，也可拖放手动排序。主评级页左侧的已评级学生列表使用同一套共享排序设置，因此编辑器与视频工作室中的手动顺序和排序方式会保持一致。

五个维度权重提供两种编辑方式。默认的“精细百分比”使用固定 `0-100%` 刻度和明确的“未分配权重”预算：提高某个维度时只会消耗未分配预算，降低某个维度时会把释放的权重放回未分配预算，其他维度不会自动变化。百分比输入框只会在失焦或按 Enter 时提交，输入过程中的半成品数值不会立即生效；超过可用预算的数值只会夹紧当前编辑的维度，并且输入框会立即回到实际提交后的数值。当未分配权重不为 `0%` 时，自动综合分、卡片导出和视频渲染会暂时禁用；完整分配后按 `sum(维度分 × 权重百分比) / 100` 计算。“预设权重”会恢复旧版的不计/半权重/全权重系统，适合不想细调的用户。默认仍使用全体共用权重，也可切换到单独设置模式。评级编辑器和视频工作室使用同一套共用权重设置，因此在任意页面修改都会更新同一份底层状态。已有归一化百分比 JSON 会按精细百分比导入，旧版 `none`、`half`、`full` 文件会按预设权重导入。

“维度排名”报告可以按单个评分维度从高到低或从低到高列出已评级学生，并显示方形头像、等级、学校、ID 与综合评分上下文。报告可导出为紧凑 PNG，方便检查评分一致性。

雷达动画可分别调整机械扫描、扫描余辉、数据点显示、扫描后多边形显示、信息块入场、综合评级显示、辉光和评论滚动。默认扫描时长为 1.5 秒；每个维度会在扫描线经过对应轴后，直接在最终位置淡入显示，高分 S/A 仍保留波纹强调效果。长评论会在评论块入场后开始滚动，并在“适配停留时长”模式下于淡出前抵达底部；仍可切换为固定像素/秒速度。视频评论会根据实际渲染出来的文字高度判断是否溢出，因此即使只是最后一行轻微被裁切，也会触发滚动。
